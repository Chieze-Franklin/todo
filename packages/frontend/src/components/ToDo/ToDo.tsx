...
const ToDo = () => {
  ...
  const createTask = async (text: string) => {
    ...
    setTasks([...tasks, task]);
    setInputText('');
    const res = await createTaskInDB(task);
    if (res.error) {
  ...
  const createTaskUsingAI = (text: string) => {
    if (text.trim() === '') return;
    setLoading(true);
    createTaskWithAI(text, async (t) => {
      ...
      <footer className='flex flex-col my-7 '>
        <div className='flex items-center bg-gray-200 rounded-full'>
          <input type='text' placeholder='Describe your task' className='bg-transparent border-0 outline-none flex-1 h-14 pl-6 pr-2 placeholder:text-slate-600'
              ...
              onKeyUp={(e) => e.key === 'Enter' && createTask(inputText)}
          />
          <button
            className='border-none rounded-full disabled:bg-gray-200 bg-orange-600 w-14 h-14 text-white text-lg font-medium cursor-pointer flex items-center justify-center align-middle'
            disabled={loading}
            onClick={() => createTaskUsingAI(inputText)}
          >
