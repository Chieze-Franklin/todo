...
const GroupsDropDown = ({ groups, initialGroup, readonly, onAddGroup, onDeleteGroup, onSelectGroup }: Props) => {
  ...
  return (
    <div className='relative inline-block text-left'>
      <div>
        ...
        >
          {selectedGroup ? selectedGroup : DEFAULT_GROUP}
      ...
      {showDropDown && <div className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex={-1}>
        <div className="py-1" role="none">
          ...
          <a className={`block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 ${!selectedGroup || selectedGroup === DEFAULT_GROUP ? 'bg-orange-300' : ''}`} tabIndex={-1} role="menuitem" id={`menu-item-default`} onClick={() => selectGroup(DEFAULT_GROUP)}>{DEFAULT_GROUP}</a>
